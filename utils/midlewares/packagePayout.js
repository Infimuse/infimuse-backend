...
exports.packagePayout = async (req, res) => {
  ...
  if (!package) {
    return res.status(404).json({ msg: "No Package with that Id" });
  }

  const tickets = await PackageTicket.findAll({
    where: { packageClassId, ticketStatus: "ACTIVE" },
  });
  ...
  if (plan.subscription === "FreePlan") {
    ...
    // await payouts.createPayout(req, res, Recipient, paystackKey, reference);
